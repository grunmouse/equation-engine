...
- **`operators`**: Объект с операторами. Ключи — символы операторов (например, `+`, `*`). Значения — объекты с:
  - `order`: Приоритет (число, чем меньше - тем приоритетнее).
  - `arity`: Арность (число аргументов, по умолчанию 2).
  - `fix`: Тип ('prefix', 'infix', 'postfix', по умолчанию 'infix' для бинарных).
  - `right`: `true` для правоассоциативных (например, `**` для возведения в степень).
  - `sign`: Символ операции, если отличается от ключа (нужно для создания перегруженных операторов, например, унарный минус).
  - `com`: Команда для машины, если отличается от ключа (нужно для создания псевдонимов команд, например `**` и `^` для возведения в степень).

- **`functions`**: Объект с функциями. Ключи — имена функций. Значения — объекты с `arity` (арность). Разрешена арность -1, означающая, что арность будет вычислена в рунтайме.
...
2. **Вычисление**: `let result = mashine(poliz, env);` — Получаете результат. Если стек имеет несколько элементов, возвращается массив.

Параметр `env` - окружение. Он передаётся в
- Парсеры литералов
- Функцию получения переменных
- Реализации команд
